...
def save_model(model, optimizer, step, base_dir, last_step):
    # Define the iteration-specific directory
    iteration_dir = os.path.join(base_dir, f'step_{step}')
    ...
    try:
        ...
        model_to_save.config.to_json_file(out_fn_to_fp('config.json'))
        # Save model weights
        torch.save(model_to_save.state_dict(), out_fn_to_fp('model_weights.pt'))
        # Save optimizer state
        torch.save(optimizer.state_dict(), out_fn_to_fp('optimizer.pt'))
        # Save step count
        with open(out_fn_to_fp('step.pkl'), 'wb') as f:
            pickle.dump(step, f)
        print(f"Model and related data saved successfully to {iteration_dir}")
...
def train(args):
  ...
  if not args.eval_only:
    ...
    train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=args.train_batch_size, drop_last=True, pin_memory=False)
  eval_sampler = SequentialSampler(eval_data)
  eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size, drop_last=True, pin_memory=False)
      ...
      for batch in train_dataloader:
        ...
        if int(elapsed / args.train_eval_secs) > num_save:
          ...
          if best_eval_loss is None or eval_dict['eval_infill_loss'] < best_eval_loss:
            ...
            # Estimate the size of the model configuration
            config_size = sys.getsizeof(pickle.dumps(model_to_save.config))
            # Estimate the size of the model weights
            model_size = sum(p.numel() * p.element_size() for p in model_to_save.state_dict().values())
            # Estimate the size of the optimizer state
            optimizer_size = sum(p.numel() * p.element_size() for p in optimizer.state_dict().values() if isinstance(p, torch.Tensor))
            # Estimate the size of the step count
            step_size = sys.getsizeof(pickle.dumps(step))
            # Total estimated size in bytes
            total_size = config_size + model_size + optimizer_size + step_size
            # Convert to a more readable format (e.g., MB)
            total_size_mb = total_size / (1024 * 1024)

            print(f"Estimated disk space required: {total_size_mb:.2f} MB")
            save_model(model, optimizer, step, args.train_dir, last_best_step)
